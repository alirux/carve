...
class HtmlReporterTest {
    ...
    private static String write(ClassGraphModel model) throws IOException {
        StringWriter w = new StringWriter();
        new HtmlReporter().write(w, model);
        return w.toString();
    }

    @Test
    void GIVEN_a_class_model_WHEN_writing_html_THEN_template_is_kept_and_placeholder_is_replaced_with_data()
            throws IOException {
        CallGraph cg = new CallGraph();
        cg.addEdge(method("app", "A", "m").build(), method("app", "B", "n").build());
        ...
        assertThat(html).contains("app.A");           // a node id from the model
        assertThat(html).contains("\"multiProject\":false");
    }

    @Test
    void GIVEN_a_class_model_WHEN_writing_html_THEN_the_copyright_note_is_embedded() throws IOException {
        CallGraph cg = new CallGraph();
    ...
    @Test
    void GIVEN_a_class_model_WHEN_writing_html_THEN_the_visible_footer_and_its_data_are_present()
            throws IOException {
        CallGraph cg = new CallGraph();
        cg.addVertex(method("app", "A", "m").build());
        ...
        assertThat(html).contains("\"disclaimer\":"); // the field is present…
        assertThat(html).contains("with no warranty"); // …with the disclaimer text (JSON-escapes its own quotes)
    }
