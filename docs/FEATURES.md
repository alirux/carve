...
| `class-graph.gexf` | edge columns `edgeKind` (`direct`/`cha`), `chaWeight` and `implFanOut` — filterable in Gephi, the last as a numeric *Range* filter |
| `class-edges.csv` | an `edgeKind` column (`direct`/`cha`) plus `chaWeight` and `implFanOut`, one row per class-to-class edge |
| `call-graph.dot` | dashed edges, grey when CHA resolved the only implementation and amber when it was choosing between several ([CHA.md §6b](CHA.md#6b-how-much-an-inferred-edge-is-worth-implfanout)); the tooltip carries the count |
...
A collapsed edge counts as `cha` only when **every** underlying method call was inferred; one real call site makes it `direct`. Read as a dependency map — comparing against `pom.xml` / `build.gradle`, for instance — exclude the edges with `implFanOut > 1`, not every `cha` edge: an inference that resolved the only implementation of an interface is a real dependency, and dropping it understates the coupling. See [CHA.md §6b](CHA.md#6b-how-much-an-inferred-edge-is-worth-implfanout).

See [CHA.md](CHA.md) for the full behaviour: why the over-approximation is deliberate, the phantom couplings it creates on multi-`--source` runs, and which outputs filter the ambiguous edges (the coupling metrics do; the raw exports do not).
...
One shape is deliberately withheld from `extractionCandidates`: a **presentation-only** package, whose Spring components are *all* controllers (`@Controller`/`@RestController`). Such a package has low `Ca` by construction — nothing depends on a controller — so it matches the extractable shape without being a bounded context; it is the edge of the system, not a service waiting to be peeled off. It is left unclassified rather than ranked. A package that mixes controllers with service or repository logic is not withheld — that judgement is left to the reader (see [AI.md §3](AI.md#3-practices-that-earned-their-keep)).
...
**`class-graph.html`** — a self-contained 3D viewer built on [3d-force-graph](https://github.com/vasturiano/3d-force-graph) (WebGL/Three.js). Open it in a browser; nothing to install. Supports rotate / zoom / pan, per-project filtering, a "risks only" filter, risk highlighting, and class search.

Edges are drawn by provenance: pale grey for a call site in the source, blue for an edge CHA inferred where the interface had a single implementation (sound), amber for one where it had several (at most one of them is real). The **Inferred edges** control switches between three views — `show all`, `hide ambiguous only` (the dependency view: call sites plus exact inferences), and `hide all inferred` (call sites only). The two hiding modes are not nested: an edge with at least one real call site survives *hide all inferred*, so a few amber edges can remain. The header counts inferred and ambiguous edges separately. See [CHA.md §6b](CHA.md#6b-how-much-an-inferred-edge-is-worth-implfanout) for why "inferred" and "doubtful" are not the same set. In multi-project mode nodes are coloured by project so the module boundary is visible; otherwise by role (transactional / external / both).
...
**`package-graph.html`** — same 3D viewer technology as `class-graph.html`. Includes a **"Group by project" checkbox** that clusters packages by module using a secondary spring layout, making cross-project dependencies immediately visible. Edge colouring and the **Inferred edges** control work exactly as in `class-graph.html`, with the fan-out aggregated from the class edges underneath each coupling (worst case wins).
...
- **CHA (class hierarchy analysis) across independent `--source` roots** — the type universe spans every `--source` root at once, so a call on a shared interface (especially `Function`/`Consumer`/`Supplier`) gains edges to implementations in projects the caller has no build dependency on. The package-coupling metrics exclude these (they are ambiguous, `implFanOut > 1`), so they no longer inflate the report; but the raw `class-edges.csv`, GEXF and *show all* HTML view still list them, tagged, so exclude `implFanOut > 1` there before reading the raw graph as a build-dependency map. See [CHA.md](CHA.md).
- **Exactly-resolved inferred edges count toward coupling** — the coupling metrics filter the *ambiguous* inferred edges but keep the exactly-resolved ones (`implFanOut = 1`, the interface had a single implementation in the analysed source). Those are sound dependencies when the implementation really is unique, but an implementation living outside the `--source` roots, or supplied by a dynamic proxy, is invisible to that count — see [CHA.md §6b](CHA.md#6b-how-much-an-inferred-edge-is-worth-implfanout). The transaction and lock analyses, by contrast, keep *every* inferred edge on purpose.
