...
- **Coupling metrics include inferred edges** — the package-coupling section (afferent/efferent coupling, instability, and the modernisation hotspots derived from them) is computed over *every* edge, CHA-inferred ones included. `summary.chaEdges` reports how many edges are inferred overall, but not which couplings rest on them: to find out, filter `class-edges.csv` on `implFanOut > 1`, range-filter the same column in the GEXF, or pick *hide ambiguous only* in the HTML viewers. Filtering on `edgeKind=cha` instead drops the inferred edges that resolved the only implementation of an interface, which are sound — see [CHA.md §6b](CHA.md#6b-how-much-an-inferred-edge-is-worth-implfanout).
